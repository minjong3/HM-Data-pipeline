...
# 무결성 테스트를 위한 쿼리
partition_query = "SELECT COUNT(*) AS partition_cnt FROM pn_transactions"
source_query = "SELECT COUNT(*) AS source_cnt FROM transactions_source"
# 중복값 테스트를 위한 쿼리
duplication_query = "SELECT paymentnumber, COUNT(*) AS duplication_cnt FROM pn_transactions \
    GROUP BY paymentnumber HAVING COUNT(*) > 1;"
...
dag_id = 'pn_transaction_athena'
dag = DAG(
    dag_id=dag_id,
    default_args=DEFAULT_ARGS,
    start_date=datetime(2023, 12, 6),
    schedule_interval='30 5 * * *', #11시 30분 시작
    tags=["athena"],
    catchup=False,
...
sensor = ExternalTaskSensor(
    task_id='wait_for_transaction_slack_dag',
    external_dag_id='transaction_slack_dag',
...
duplication_query >> sleep_task3 >> get_duplication_athena >> slack_task3 >> msg_athena