...
# 무결성 테스트를 위한 쿼리
partition_query = "SELECT COUNT(*) AS partition_cnt FROM customers"
source_query = "SELECT COUNT(*) AS source_cnt FROM customers_source"
# 중복값 테스트를 위한 쿼리
duplication_query = "SELECT customer_id, COUNT(*) AS duplication_cnt FROM customers \
...
dag_id = 'customer_athena'
dag = DAG(
    dag_id=dag_id,
    default_args=DEFAULT_ARGS,
    start_date=datetime(2023, 12, 6),
    schedule_interval='20 5 * * *', #11시 20분 시작
    tags=["athena"],
    catchup=False,
...
sensor = ExternalTaskSensor(
    task_id='wait_for_customer_slack_dag',
    external_dag_id='customer_slack_dag',
